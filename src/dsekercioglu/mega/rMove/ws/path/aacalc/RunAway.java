...
public class RunAway extends AttackAngleCalculator {
    ...
    public RunAway(double battleFieldWidth, double battleFieldHeight) {
    ...
    public double getAttackAngle(Wave wave, Point2D.Double location, Point2D.Double enemyLocation, double currentAngle, double targetVelocity) {
        ...
        double targetAngle = MoveUtils.absoluteBearing(enemyLocation, location) + Math.PI / 2;
        double dir = Math.signum(targetVelocity);
        if (dir != 0) {
            double distance = location.distance(enemyLocation);
            double distanceDifference = MoveUtils.limit(-PREFERRED_DISTANCE, distance - PREFERRED_DISTANCE, PREFERRED_DISTANCE);
            double extraAngle = MoveUtils.signedSq(distanceDifference / PREFERRED_DISTANCE) * AGGRESSION;
